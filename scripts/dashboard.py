...
st.title("🛒 Superstore 電商營運分析 Dashboard")
...
selected_cats = st.sidebar.multiselect("品類", categories, default=categories)

regions = df["Region"].unique().tolist()
selected_regions = st.sidebar.multiselect("地區", regions, default=regions)
...
col1.metric("💰 總銷售額",    f"${total_sales:,.0f}")
col2.metric("📈 總利潤",      f"${total_profit:,.0f}")
col3.metric("📊 平均利潤率",  f"{avg_margin:.1%}")
col4.metric("📦 訂單數",      f"{order_count:,}")
...
with row1_col1:
    ...
    monthly_q = """
        ...
        ORDER BY Month
    """
    monthly = run_sql(filtered, monthly_q)
    fig = go.Figure()
    fig.add_bar(x=monthly["Month"], y=monthly["Sales"],
                name="Sales", marker_color="#4C9BE8", opacity=0.7)
...
# 品類 vs 利潤率
with row1_col2:
    st.subheader("🏷️ 品類利潤率分析")
    cat_q = """
        SELECT Category, Sub-Category AS SubCategory,
               ROUND(SUM(Sales), 0) AS Sales,
               ROUND(AVG("Profit Margin") * 100, 1) AS AvgMarginPct
        FROM orders
        GROUP BY Category, [Sub-Category]
        ORDER BY AvgMarginPct DESC
    """
    # 用 pandas 做（避免 Sub-Category 欄名問題）
    ...
    fig2 = px.bar(cat_data, x="AvgMarginPct", y="Sub-Category",
                  color="Category", orientation="h",
                  color_discrete_sequence=px.colors.qualitative.Set2)
    fig2.add_vline(x=0, line_dash="dash", line_color="red")
    fig2.update_layout(height=350, margin=dict(t=20), showlegend=True)
...
# 折扣 vs 利潤率
with row2_col1:
    st.subheader("⚠️ 折扣率 vs 利潤率（關鍵洞察）")
    fig3 = px.scatter(filtered, x="Discount", y="Profit Margin",
                      color="Category", opacity=0.5, size="Sales",
                      size_max=15,
                      color_discrete_sequence=px.colors.qualitative.Set1)
    fig3.add_hline(y=0, line_dash="dash", line_color="red",
                   annotation_text="損益兩平線")
    fig3.update_layout(height=350, margin=dict(t=20))
    st.plotly_chart(fig3, use_container_width=True)

# 地區表現
with row2_col2:
    st.subheader("🗺️ 地區銷售與利潤")
    ...
    fig4 = px.bar(region_data, x="Region", y=["Sales", "Profit"],
                  barmode="group",
                  color_discrete_sequence=["#4C9BE8", "#FF6B6B"])
...
col_b.warning(f"📌 **虧損品項**\n\n**{worst_sub}** 是總利潤最差的子品類，建議檢討定價或下架策略。")
